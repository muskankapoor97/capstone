...
export default function Laundry(){
  ...
  useEffect(() => {
    const getMachine = async () => {
      try {
        const { data } = await axios.get(`http://localhost:5050/laundry/${id}`);
        setMachine(data["laundryData"]);
      } catch (error) {
        console.log("Error", error);
      }
    };
  ...
  const myform=event.currentTarget
   ...
   const isMachineIDValid = myform.machineid.value;
   ...
   if (!isMachineIDValid) {
    myform.machineid.style.border = "1px solid yellow";
    document.getElementById("MachineID-Valid").style.display = "block";
  } else {
    myform.machineid.style.border = "1px solid #bdc5d5";
    document.getElementById("MachineID-Valid").style.display = "none";
  }
   if(isFirstNameValid && isLastNameValid &&isDateValid&&isUnitNumberValid&&isEmailValid&&isMachineIDValid){
    const updatedBooking = {
      id: event.target.machineid.value,
      name:machine.name,
      type:"red",
      status:"Running",
    ...
    axios
      .put(`http://localhost:5050/laundry/${id}`, updatedBooking)
        ...
        <div className="form__details">
          <div className="form__detail-nameContainer">
           ...
           <div className="form__detail">
            ...
            <div className="form__detail">
              <label className="form__label">Machine Id</label>
              <input className="form__input" defaultValue={id} name="machineid"></input>
              <div className="MachineID-Valid" id="MachineID-Valid">
              <span className="MachineID-Valid__text">
                This field is required
              </span>
            </div>
            </div>
